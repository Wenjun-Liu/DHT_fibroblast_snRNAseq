...
Cells that have unique feature counts over 4,000 or less than 200 mitochondrial counts were filtered out. 

```{r}
fibroblast <- subset(fibroblast, subset = nFeature_RNA > 200 & nFeature_RNA < 4000)
```
...
UMAP_clu <- DimPlot(fibroblast, 
                    reduction = "umap", label = TRUE, label.size = 16) + 
...
#write_csv(cluster_marker, here::here("output/cluster_marker_unintegrated.csv"))
cluster_marker  %>%
    split(f = .$cluster) %>%
...
The expressions of the 2 genes were compared across all cells.

```{r}
seurat_count <- GetAssayData(object = fibroblast, slot = 'data')
temp <- seurat_count %>%
    .[rownames(.) %in% c("ENG", "DPP4"),] %>%
    as.matrix() %>%
    t() %>%
    as.data.frame() %>%
    ...
    mutate(
        label = case_when(
            DPP4 == 0 & ENG != 0 ~ "Intra-lobular", 
            DPP4 != 0 & ENG == 0 ~ "Inter-lobular"),
         label = ifelse(
             is.na(label), "Undefinitive", label
         )
    ) 
fibroblast$location_label <- temp %>%
    .[match(colnames(fibroblast), .$cell),] %>%
    pull(label)
```
...
chap5_3a <- temp %>%
    pivot_longer(
        cols = c("DPP4", "ENG"),
        names_to = "gene_name", 
        values_to = "Log-norm Count"
    ) %>%
    mutate(
        gene_name = ifelse(
            gene_name == "ENG", "*ENG*", "*DPP4*"
        )
    ) %>%
    ggplot(
        aes(gene_name, `Log-norm Count`, fill = patient)
    ) +
    geom_boxplot() +
    scale_fill_manual(
        values = patient_col, 
...
chap5_3b <- temp %>%
    ggplot(
        aes(ENG, DPP4, color = label)
    ) +
    geom_point() +
    scale_color_manual(
        values = c(
            "Intra-lobular" = "#ACA4E2", 
            "Inter-lobular" = "#5CBD92", 
            "Undefinitive" = "gray"
        ), 
        name = "Subtype"
    ) +
    labs(
        x = "Log-norm Count (*ENG*)",
        y = "Log-norm Count (*DPP4*)"
    ) +
    geom_abline(slope = 1,
                size = 2, 
                linetype = "dashed", 
    ...
    guides(colour = guide_legend(override.aes = list(size=10)))
chap5_3c <- DimPlot(fibroblast, group.by = "location_label", pt.size = 1.5) +
    scale_color_manual(
        values = c(
            "Intra-lobular" = "#ACA4E2", 
            "Inter-lobular" = "#5CBD92", 
...
```{r, fig.height=10, fig.width=14, fig.cap="*Figure 3. UMAP plots showing the expressions of (A) lobular marker ENG and (B) interlobular marker DPP4.*"}
(((chap5_3a + labs(tag = 'A')) / (chap5_3c + labs(tag = 'C'))) | (chap5_3b + labs(tag = 'B'))) +
    plot_layout(
        guides = "collect", 
        widths = c(0.4, 0.6)) &
    theme(
...
Most of the sequenced cells were predicted to have either a intra-lobular origin or undefinitive, while only a small proportion of cells may have been interlobular. 
...
# chap5_fig3 <-  (((chap5_3a + labs(tag = 'A')) / (chap5_3c + labs(tag = 'C'))) | (chap5_3b + labs(tag = 'B'))) +
#     plot_layout(
#         guides = "collect",
#         widths = c(0.4, 0.6)) &
#     theme(
...
#     width = 700, height =350, units='mm', res = 300
# )
# chap5_fig3
# dev.off()
```
...
#     ) +
#     geom_bar(stat="identity", position=position_dodge()) +
#     # geom_text(aes(label=n_treat), vjust=1.6, color="white",
