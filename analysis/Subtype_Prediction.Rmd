...
The prediction of cell label was performed using the `SingleR()` function with the log-normed matrix as input for the test dataset. Since this is an unbiased cell type annotation analysis, all cell types were included in the reference dataset. 
...
p1 <- DimPlot(fibroblast, 
    ...
    ggtitle("") +
    scale_color_brewer(palette = "Dark2", na.value = "grey", direction = -1) + 
...
p2 <- DimPlot(fibroblast, 
    ...
    ggtitle("") +
    scale_color_brewer(palette = "Set3",  na.value = "grey") + 
    guides(colour = guide_legend(override.aes = list(size=10)))
...
The `Seurat` object was broken into parts so it could be converted into an `Annobject` in python. 
```{r}
# fibroblast$barcode <- colnames(fibroblast)
# fibroblast$UMAP_1 <- fibroblast@reductions$umap@cell.embeddings[,1]
# fibroblast$UMAP_2 <- fibroblast@reductions$umap@cell.embeddings[,2]
# write.csv(fibroblast@meta.data, file=here::here("output/metadata.csv"), quote=F, row.names=F)
# counts_matrix <- GetAssayData(fibroblast, assay='RNA', slot='counts')
# Matrix::writeMM(counts_matrix, file=here::here("output/counts.mtx"))
# write.csv(fibroblast@reductions$pca@cell.embeddings,
#           file=here::here("output/pca.csv"),
#           quote=F, row.names=F)
# write.table(
#   data.frame('gene'=rownames(counts_matrix)),file=here::here("output/gene_names.csv"),
...
png(
    ...
    width = 600, height =270, units='mm', res = 300
)
chap5_fig5
dev.off()
```
...
# pdf(
#     "/Users/wenjunliu/PhD_thesis/Images/Chapter_05/chap5_sup3.pdf",
#     height = 25,
#     width = 30)
# chap5_sup3
# dev.off()
